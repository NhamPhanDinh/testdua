...
public class GPGMultiplayerManager : AbstractManager
{
	...
	// Fired when the waiting room completes. The bool indicates if the waiting room succeeded (a room was joined)
	public static event Action<bool> onWaitingRoomCompletedEvent;
	
	// Fired when the invitation inbox screen completes. The bool indicates if a room was joined.
	public static event Action<bool> onInvitationInboxCompletedEvent;

	// Fired when the player selector screen completes. The bool indicates if a room was created.
	public static event Action<bool> onInvitePlayersCompletedEvent;


	// Room Events (RoomUpdateListener on the native side)

	// Fired when the client attempts to join a real-time room. Calling showWaitingRoom is allowed when this event fires.
	public static event Action<GPGRoom,GPGRoomUpdateStatus> onJoinedRoomEvent;
	
	// Fired when the client attempts to leaves the real-time room
	...
	// Fired when when the client attempts to create a real-time room. Calling showWaitingRoom is allowed when this event fires.
	...
	{
		onRealTimeMessageReceivedEvent.fire( senderParticipantId, message );
	}
